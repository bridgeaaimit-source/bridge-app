...
const filePath = path.join(__dirname, 'app', 'aptitude', 'page.js');
...
// We'll search for 'return (' at the bottom.
const returnIndex = code.lastIndexOf('return (');
if (returnIndex !== -1) {
  const codeBefore = code.substring(0, returnIndex);
      ...
      <AppShell hideNavigation={stage === "test" || stage === "loading-test"}>
        <div className="min-h-screen text-slate-800 font-sans selection:bg-teal-500 selection:text-white">
                            ...
                            <div className={`w-8 h-8 rounded-xl border flex items-center justify-center transition-all \${
                              day.completed 
                                ...
                                : day.date === new Date().toISOString().split('T')[0]
                                  ? "bg-amber-50 border-amber-500 text-amber-705 font-bold"
                ...
                <Card className="flex flex-col justify-between p-6">
                  <div>
                    <h3 className="text-sm font-bold text-slate-800 tracking-wider uppercase flex items-center gap-2 mb-4">
                      <TrophyIcon className="w-4 h-4 text-indigo-505" /> Leaderboard (Top 5 Today)
                    </h3>
                      ...
                      {topScorers.length > 0 ? (
                        topScorers.map((scorer, idx) => (
                          <div key={idx} className="flex items-center justify-between gap-3 p-2 bg-slate-50/50 rounded-xl border border-slate-100">
                            <div className="flex items-center gap-2">
                              <span className={`w-4 text-center font-bold text-xs \${idx === 0 ? "text-amber-500" : idx === 1 ? "text-indigo-450" : "text-slate-400"}`}>
                                {idx + 1}
                              </span>
                            ...
                            <div className="text-right">
                              ...
                              <span className="text-[9px] text-slate-450 block leading-none">Bridge Score</span>
                            </div>
                          </div>
                        ))
                      ) : (
                      ...
                      >
                        ...
                        <div className="space-y-2 pt-3 border-t border-slate-100">
                          ...
                          <div className="flex justify-between text-xs font-medium">
                            ...
                            <span className="font-semibold text-slate-805">{comp.duration} Mins</span>
                          </div>
                          <div className="flex justify-between text-xs font-medium">
                        ...
                        <div className="mt-4 flex flex-wrap gap-1">
                          {Object.keys(comp.sections).map((sec) => (
                            <span key={sec} className="text-[8px] font-bold bg-slate-550 border border-slate-200 text-slate-500 px-1.5 py-0.5 rounded uppercase">
                              {sec}
                            </span>
                          ))}
                        </div>
              ...
              <div className={`p-4 md:p-8 max-w-4xl mx-auto space-y-6 min-h-[90vh] transition-all duration-300 \${
                ...
                <div className={`rounded-2xl border p-4 px-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all duration-300 \${
                  ...
                  <div className="flex items-center gap-4 flex-wrap sm:flex-nowrap">
                    ...
                    {/* XP Counter */}
                    <div className="text-xs font-bold bg-teal-50 border border-teal-200 px-3 py-1.5 rounded-full text-teal-650">
                      +{xp} XP
                    </div>
                    ...
                    {!isDailyChallengeMode && (
                      <div className="flex items-center gap-1.5 border-l border-slate-200 pl-4">
                        ...
                        >
                          ...
                          {isTimeFrozen && (
                            <span className="absolute -top-1.5 -right-1.5 bg-cyan-605 text-white text-[8px] font-bold w-4 h-4 rounded-full flex items-center justify-center">
                              {freezeDurationLeft}
                            </span>
                          )}
                        </button>
                    ...
                    {q?.options.map((opt, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full py-4 px-6 text-left rounded-xl border text-sm transition-all duration-155 flex items-center justify-between \${
                            showCorrect 
                              ...
                              : showWrong 
                                ...
                                : selectedOption !== null 
                                  ...
                                  : "bg-slate-50/50 border-slate-200 hover:border-teal-500 text-slate-750 hover:bg-teal-50/10"
                          }`}
                        >
                          <span>{opt}</span>
          ...
          {stage === "results" && results && (
            <div className="p-6 md:p-12 max-w-4xl mx-auto space-y-8 animate-in fade-in duration-505">
              ...
              <div className="text-center space-y-2 border-b border-slate-100 pb-6">
                ...
                <p className="text-sm text-slate-505 max-w-sm mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {/* Stats Summary Panel */}
                <Card className="md:col-span-2 grid grid-cols-2 gap-6 items-center p-6 bg-slate-50/55">
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-450 block uppercase font-bold tracking-wider">XP Reward</span>
                    <div className="text-xl font-bold text-teal-650">+{results.xpEarned} XP</div>
                    <span className="text-[9px] text-slate-450 block leading-none">Bridge Score Impact</span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-450 block uppercase font-bold tracking-wider">Benchmarking</span>
                    <div className="text-xl font-bold text-indigo-650">Top {100 - results.percentile}%</div>
                    <span className="text-[9px] text-slate-450 block leading-none">Beat {results.percentile}% candidates</span>
                  </div>
                  <div className="space-y-1">
                    ...
                    <span className="text-[9px] text-slate-450 block leading-none">Active minutes logged</span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-[10px] text-slate-455 block uppercase font-bold tracking-wider">Pacing Ratio</span>
                    <div className="text-xl font-bold text-slate-850">
...
`;
  code = codeBefore + newReturnBlock;
}

fs.writeFileSync(filePath, code, 'utf8');
console.log("Aptitude page.js successfully modified!");
