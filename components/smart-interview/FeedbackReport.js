              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-white rounded-2xl p-8 border border-gray-100 shadow-[0_4px_20px_rgba(13,148,136,0.06)] flex flex-col justify-center items-center text-center">
                  ...
                  <p className={`text-6xl font-bold mb-3 ${
                    state.integrity.integrityScore >= 80 ? 'text-[#0D9488]' :
                    state.integrity.integrityScore >= 50 ? 'text-yellow-600' : 'text-red-500'
                  ...
                  <span className={`inline-flex items-center px-5 py-2 rounded-full text-sm font-bold ${
                    state.integrity.integrityScore >= 80 ? 'bg-teal-50 text-[#0D9488] border border-teal-100' :
                    state.integrity.integrityScore >= 50 ? 'bg-yellow-50 text-yellow-700 border border-yellow-100' :
                    'bg-red-50 text-red-700 border border-red-100'
                  }`}>
                    {integrityScore >= 80 ? '🔒 Integrity Passed' :
                     state.integrity.integrityScore >= 50 ? '⚠️ Warnings Triggered' : '❌ Flagged for Review'}
                  </span>
                  {violations.length > 0 && (
                ...
                {Object.entries(feedback.scores || {}).map(([key, value]) => {
                  ...
                  return (
                    <div key={key} className="bg-white rounded-2xl p-5 shadow-[0_4px_20px_rgba(13,148,136,0.06)] border border-gray-100">
                      ...
                      <p className="text-2xl font-bold text-[#0D9488] mb-2" style={{fontFamily:'Syne,sans-serif'}}>{scoreValue}<span className="text-sm text-gray-400">/10</span></p>
