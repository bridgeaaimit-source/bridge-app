...
import { calcTTSCostINR } from './modelPricing';

const GEMINI_VOICE = "Aoede";
const ELEVENLABS_VOICE_ID = process.env.ELEVENLABS_VOICE_ID || "21m00Tcm4TlvDq8ikWAM";

// Initialize global in-memory cache if not already present
global.ttsMemoryCache = global.ttsMemoryCache || new Map();

export async function generateAndCacheTTS(text, userId = 'anonymous') {
  ...
  if (global.ttsMemoryCache.has(hash)) {
    const cached = global.ttsMemoryCache.get(hash);
    isCacheHit = true;
    audioSource = 'cache';
    generationTimeMs = performance.now() - startTotal;
    logAnalytics(text, isCacheHit, generationTimeMs, audioSource, userId);
    return { buffer: cached.buffer, contentType: cached.contentType, isCacheHit };
  }
  ...
  if (bucket) {
    ...
    if (exists) {
      ...
      global.ttsMemoryCache.set(hash, { buffer: cacheBuffer, contentType: 'audio/wav' });
  ...
  // Cache it
  if (finalAudioBuffer) {
    global.ttsMemoryCache.set(hash, { buffer: finalAudioBuffer, contentType });
    if (bucket) {
      try {
...
function logAnalytics(text, isCacheHit, generationTimeMs, audioSource, userId = 'anonymous') {
  if (!adminDb) return;
  
  const charCount = text.length;
  const { costINR, modelSlug } = calcTTSCostINR(charCount, audioSource);
  ...
  // Log to tokenUsage/voice/{date}/{uid} for dashboard integration
  const today = new Date().toISOString().split('T')[0];
  const uid = userId || 'anonymous';
  
  adminDb.collection('tokenUsage')
    .doc('voice')
    .collection(today)
    .doc(uid)
    .set({
      userId: uid,
      date: today,
      tts_chars: admin.firestore.FieldValue.increment(charCount),
      tts_cost_inr: admin.firestore.FieldValue.increment(costINR),
      lastUsed: admin.firestore.FieldValue.serverTimestamp()
    }, { merge: true })
    .catch(err => console.error("Voice daily usage log failed:", err));
}
