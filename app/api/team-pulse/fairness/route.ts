...
export async function GET() {
  ...
  try {
    ...
    const rateF = femaleCands.length ? femaleCands.filter((c) => isShortlisted(c.stage)).length / femaleCands.length : 0.45;
    const rateM = maleCands.length ? maleCands.filter((c) => isShortlisted(c.stage)).length / maleCands.length : 0.52;
    ...
    const promoF = femaleEmp.length ? femaleEmp.filter((e) => e.perf >= 4).length / femaleEmp.length : 0.28;
    const promoM = maleEmp.length ? maleEmp.filter((e) => e.perf >= 4).length / maleEmp.length : 0.32;
    ...
    return NextResponse.json({ error: "Failed to fetch fairness audit metrics" }, { status: 500 });
  }
}
